...
if [ "$CRUMB" = true ] ; then
    ...
    while IFS=',' read STUDENT GIT_URL; do 
        ...
        curl -s -X POST "${JENKINS_URL}/job/${STUDENT}/job/practica${PRACTICA}_${STUDENT}/createItem?name=${JOB_NAME}" --user ${USER}:${PASS} --data-binary "@/tmp/${JOB_NAME}_${STUDENT}" -H "Content-Type:text/xml" -H "${CRUMB_KEY}"
    done < ${FILE}
else
    while IFS=',' read STUDENT GIT_URL; do 
        # Modify the job template of Jenkins to include the repo of each student
        cat ${JOB_TEMPLATE} | sed "s/#URL#/$(echo $GIT_URL | sed -e 's/\\/\\\\/g; s/\//\\\//g; s/&/\\\&/g')/g" >> /tmp/${JOB_NAME}_${STUDENT}
        # Create job in Jenkins for each student
        curl -s -X POST "${JENKINS_URL}/job/${STUDENT}/job/practica${PRACTICA}_${STUDENT}/createItem?name=${JOB_NAME}" --user ${USER}:${PASS} --data-binary "@/tmp/${JOB_NAME}_${STUDENT}" -H "Content-Type:text/xml"
    done < ${FILE}
fi
