...
./exercise10_bin < marks.txt > ex10_out.txt
for mark in $marks
do
...
alu_min=$(cat ex10_out.txt|grep "Minimum"|grep -Eo '[+-]?[0-9]+([.][0-9]+)?')
alu_max=$(cat ex10_out.txt|grep "Maximum"|grep -Eo '[+-]?[0-9]+([.][0-9]+)?')
alu_pas=$(cat ex10_out.txt|grep "Passed"| tr -dc '0-9')
alu_ave=$(cat ex10_out.txt|grep "Average mark"|grep -Eo '[+-]?[0-9]+([.][0-9]+)?')
alu_up_ave=$(cat ex10_out.txt|grep "higher than the mark average"| tr -dc '0-9')
alu_lo_ave=$(cat ex10_out.txt|grep "lower than the mark average"| tr -dc '0-9')
